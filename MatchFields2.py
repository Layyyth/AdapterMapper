import json
import re
import numpy as np
from sklearn.metrics.pairwise import cosine_similarity
from sentence_transformers import SentenceTransformer
import faiss

# === CONFIG ===
ob_json_path = "data/ob_field_schema.json"
bank_json_path = "data/bankTEST.json"

# === Load OB Fields ===
with open(ob_json_path, "r", encoding="utf-8") as f:
    ob_fields = json.load(f)

# === Clean and Build OB Text Corpus ===
def build_ob_text(entry):
    schema = entry.get("schema", "")
    field = entry.get("field", "")
    description = entry.get("description", "")
    path = entry.get("path", "")
    method = entry.get("method", "")
    context = f"{method} {path}".strip()
    full = f"{context} {schema}.{field}: {description}".strip()
    return re.sub(r'\s+', ' ', full)

ob_texts = [build_ob_text(f) for f in ob_fields]

# === Load & Flatten Bank Payloads ===
def extract_field_paths(data, parent=""):
    fields = []
    if isinstance(data, dict):
        for k, v in data.items():
            full_key = f"{parent}.{k}" if parent else k
            fields.extend(extract_field_paths(v, full_key))
    elif isinstance(data, list):
        if data:
            fields.extend(extract_field_paths(data[0], parent + "[]"))
    else:
        fields.append(parent)
    return fields

with open(bank_json_path, "r", encoding="utf-8") as f:
    bank_payload = json.load(f)

bank_field_names = []

for endpoint, content in bank_payload.items():
    payload_key = "response" if "response" in content else "request"
    payload = content.get(payload_key, {})
    extracted = extract_field_paths(payload)
    bank_field_names.extend(extracted)

# === Abbreviation expansion ===
abbreviation_map = {
    "acc": "account", "acct": "account", "usr": "user", "id": "identifier",
    "nm": "name", "cnst": "consent", "amt": "amount", "txn": "transaction",
    "num": "number", "ref": "reference", "cred": "creditor", "deb": "debtor",
    "auth": "authorisation", "curr": "currency", "dt": "date", "exp": "expiration",
    "exch": "exchange", "multi": "multi", "proxy": "proxy", "addr": "address",
    "post": "postal", "prod": "product", "issuer": "issuer", "status": "status",
    "upd": "update", "info": "information", "ben": "beneficiary", "pmt": "payment",
    "init": "initiation", "trf": "transfer", "ind": "indicator", "val": "value", "iban": "iban"
}

def preprocess_field(name):
    name = name.replace("[]", "")
    parts = re.sub(r'([a-z])([A-Z])', r'\1 \2', name).replace("_", " ").lower().split(".")
    tokens = []
    for p in parts:
        for sub in p.split():
            tokens.append(abbreviation_map.get(sub, sub))
    return " ".join(tokens)

# === Load Model + Embed OB fields ===
print("🔍 Embedding OB fields...")
model = SentenceTransformer("BAAI/bge-large-en-v1.5")
ob_embeddings = model.encode(ob_texts, convert_to_numpy=True)

# === Build FAISS Index ===
embedding_dim = ob_embeddings.shape[1]
index = faiss.IndexFlatL2(embedding_dim)
index.add(ob_embeddings)

# === Matching Loop ===
high_confidence_threshold = 0.85
medium_confidence_threshold = 0.70
min_confidence_gap = 0.1

mappings = []
mismatches = []
auto_mapped_count = 0

for name in bank_field_names:
    cleaned = preprocess_field(name)
    query_embedding = model.encode([cleaned], convert_to_numpy=True)
    
    D, I = index.search(query_embedding, k=5)
    top_indices = I[0]
    top_scores = cosine_similarity(query_embedding, ob_embeddings[top_indices])[0]
    sorted_indices = np.argsort(top_scores)[::-1]
    
    best_idx = top_indices[sorted_indices[0]]
    top_score = top_scores[sorted_indices[0]]
    second_score = top_scores[sorted_indices[1]] if len(sorted_indices) > 1 else -1
    gap = top_score - second_score
    
    match = ob_fields[best_idx]
    print(f"\n🔍 Bank Field: {name} → '{cleaned}'")
    
    if top_score >= high_confidence_threshold or (top_score >= medium_confidence_threshold and gap >= min_confidence_gap):
        print(f"✅ Mapped to: {match['schema']}.{match['field']} (Score: {top_score:.2f}, Gap: {gap:.2f})")
        mappings.append({
            "bank_field": name,
            "preprocessed": cleaned,
            "mapped_to": match["field"],
            "schema": match.get("schema", ""),
            "path": match.get("path", ""),
            "method": match.get("method", ""),
            "description": match.get("description", ""),
            "confidence": float(round(top_score, 3))
        })
        auto_mapped_count += 1
    else:
        print(f"⚠️ Low confidence (Score: {top_score:.2f}, Gap: {gap:.2f}) — needs review")
        suggestions = []
        for rank in sorted_indices[:3]:
            i = top_indices[rank]
            s = ob_fields[i]
            suggestions.append({
                "suggestion": s["field"],
                "schema": s.get("schema", ""),
                "path": s.get("path", ""),
                "method": s.get("method", ""),
                "description": s.get("description", ""),
                "confidence": float(round(top_scores[rank], 3))
            })
            print(f"   {rank+1}. {s['field']} (Score: {top_scores[rank]:.2f})")
        mismatches.append({
            "bank_field": name,
            "preprocessed": cleaned,
            "top_suggestions": suggestions
        })

# === Save Outputs ===
with open("field_mappings_improved.json", "w", encoding="utf-8") as f:
    json.dump(mappings, f, indent=2, ensure_ascii=False)
with open("mismatches_log_improved.json", "w", encoding="utf-8") as f:
    json.dump(mismatches, f, indent=2, ensure_ascii=False)

# === Summary ===
total = len(bank_field_names)
print(f"\n✅ Auto-mapped {auto_mapped_count}/{total} fields ({(auto_mapped_count / total) * 100:.2f}%)")
print("📄 Saved to 'field_mappings_improved.json' and 'mismatches_log_improved.json'")
